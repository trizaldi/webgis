...
<!-- Make sure you put this AFTER Leaflet's CSS -->
   ...
   <script type="text/javascript">
   	var map = L.map('map').setView([-8.184486, 113.668076], 10);
   	var layersKecamatan=[];
   	var Layer=L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token={accessToken}', {
		...
		accessToken: 'pk.eyJ1IjoidHJpemFsZGkiLCJhIjoiY2t2OHd1bWV0NXBqbDMwczdkdGYyaHJleCJ9.od3MR1E7EU2C76Oe5NdEZA'
	});
	...
	for(i=0;i<dataKecamatan.length;i++){
		...
		/*
		var data=dataKecamatan[i];
		//var KODE=dataKecamatan[i].kd_kecamatan;
		var layer={
			name: data.nm_kecamatan,
			icon: iconByName(data.warna_kecamatan),
			layer: new L.GeoJSON.AJAX(["<?=base_url()?>assets/unggah/geojson/"+data.geojson_kecamatan],
				{
					...
					style: function(feature){
						...
						console.log(data.kd_kecamatan);
						return {
							"fillColor": getColorKecamatan(KODE),
						    "weight": 1,
						    "opacity": 1
						}
